...
export default class HomePage extends Component {
  componentWillMount(){
    Storer({
      dashboard : { from: "menu", cols: "$id as id,title as text,iconCls,target,_user_access", 0: { view:1,$id:"!='/'",pid:"{is NULL}",_user_access:"1,"+window.userData._user_access} ,order:"ord" ,asc:true} ,
    //  advertise:{},
    },this);
  }
  componentDidMount() {
  ...
  render=()=> {
   // let {tags,brands,spears,general,support,edu,news} = this.state
    return <Container layout="fit" padding="0px 10px" scrollable ref='homePage'>
            {window.userData._user_access>0?
      <Container cls="rtl">
      ...
      <Title text="دسترسی سریع" icon="list" home={false} />
        <Container cls="rtl" scrollable style={{textAlign:"center"}}>
        {this.state.dashboard.map((e, i) => <Button key={i} hidden={e.id=="/Signout" || e._user_access>1}  cls="dashboard" iconAlign="top" iconCls={e.iconCls+" dashboardIcon"} text={e.text} ui="action" margin={Ext.platformTags.phone?5:30} width={Ext.platformTags.phone?140:150} height={Ext.platformTags.phone?140:150} handler={() => e.id.slice(0,4)=='http'? window.location.href=e.id : this.props.history.push(e.id)}/>)}
