...
export default class Vitrin extends PureComponent {
    ...
    Tpl = (e, i) => {
        ...
        if (e.src) {
           if(e.ago && e.price){
            ...
            return <Container shadow key={e.id || i} cls="x-round"><Container  key={e.id || i} {...this.tplProps} style={{color:"#ff0000",backgroundColor:"#000000",padding:"0px"}} padding={0}> 
            <img padding={0} src={e.src} {...this.imgProps} {...this.props.imgProps} onTap={() => { this.continue(e.target || (this.props.continue=="dialog"?i: this.props.continue + "?"+this.props.itemName+"=" + e.id)) }} height={this.props.height} style={{maxHeight:(this.props.height)+"px"}} />
           

        </Container>
        <Container margin={3} cls="rtl">
        {e.text && <center><div style={{ fontSize: "18px" ,padding:"0 20"}}>{e.text || ""}</div></center>}
        </Container>
        {/* {e.price ? <Container cls="del" style={{color:"#000000"}}> {(e.ago ? +e.ago + " تومان" : "")} </Container>:null} */}
            {e.price ? <SegmentedButton cls="ltr" width="100%" margin={1} >
                {/* <Button cls="x-zeropading" iconCls="x-fa fa-comment" iconAlign="top" ui="confirm " text={e.comments||"0"} flex={1}  handler={()=>{}} padding="0"/> */}
                {/* <Button cls="x-zeropading" iconCls="x-fa fa-heart x-zeropading" iconAlign="top" ui="decline " text={(e.like-e.dislike)||"0"} flex={1}  handler={()=>{}} padding="0"/> */}
                <Button iconCls="x-fa fa-shopping-cart" ui="action round" flex={1} pressed handler={()=>this.addCart(e.id,e.text,e.price)}/>
                <Button text={e.price + " تومان"} cls="rtl" ui="action round" flex={3} handler={()=>this.shoping(e.price,e.id)}/>
                
            </SegmentedButton>:null}
         </Container>}
        else if (e.text) {
            return <Button key={e.id || i} {...this.tplProps} text={e.text} handler={() => { this.continue(e.target || this.props.continue + "?"+this.props.itemName+"=" + e.id) }} />
        } else return <Container key={e.id || i} {...this.tplProps}>{e}</Container>
    }
    render() {
 ...
 <Dialog ref= "Dialog"
                ...
                <Carousel ref="Carousel" padding={0} layout="fit" height="100%" >
                    {settings.data.map((e,i)=> <img key={i} padding={0} src={e.src} layout="fit"   mode="img" margin="5"  />)}
                </Carousel> }
                </Dialog>
                ...
                {settings.data &&
                <Slider {...settings} rtl={false} padding={0}>
