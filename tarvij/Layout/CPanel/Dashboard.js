   ...
   // let {tags,brands,spears,general,support,edu,news} = this.state
    return <Container layout="fit" padding="0px 10px" scrollable ref='homePage'>
      ...
      <Title text="دسترسی سریع" icon="list" />
        <Container cls="rtl" scrollable><center>
        {this.state.dashboard.map((e, i) => <Button key={i}  iconAlign="top" iconCls={e.iconCls+" dashboardIcon"} text={e.text} ui="action" margin={Ext.platformTags.phone?5:30} width={Ext.platformTags.phone?140:150} height={Ext.platformTags.phone?140:150} handler={() => this.props.history.push(e.id)}/>)}
        </center>
        </Container></div>
