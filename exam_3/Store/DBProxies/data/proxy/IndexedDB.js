...
console.log("createRecord",record, i, records, options)
        if (!record.phantom) {
    ...
    updateTransaction: function(tx) {
        ...
        var options = args[args.length - 1];
        var updatedRecords = [];
        console.log("updateTransaction",tx,options)
        Ext.apply(options, {
            tx: tx,
            object_store: tx.objectStore(this.getDbName()),
    ...
    updateRecord: function(record, rI, records, options) {
        ...
        console.log(options.object_store.put)
    ...
    updateRecordSuccess: function(evt, options, record, data, modData) {
        console.log("updateRecordSuccess",evt, options, record, data, modData)
    ...
    updateRecordCallback: function(options) {
        console.log("updateRecordCallback")
        if (options.executedRecords === options.totalRecords) {
