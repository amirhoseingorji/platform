...
export default class Textbox extends Component {
    ...
    render(){
        ...
        return <Container  style={{...this.props.style,background:"linear-gradient(-90deg, "+bars[status]+" 12px, #FFFFFF 0%)"}} cls={this.props.cls} className="rtl box Texbox" shadow    >
                     ...
                     <h4 style={{margin:"0"}} className="darkblue" >{title}{status==1?<i className="fal fa-thumbtack"></i>:null}</h4>
                     <div className="blue2 boxguide" style={{minHeight:full?"300px":""}}>{description}</div>
                     <Button hidden={full} cls={"ltr orange  "} text="ادامه مطلب"  width="80" textAlign={"left"} style={{margin:"0 0 0 auto"}} ui="round" handler={()=>{this.parent.navigate('/Help?id='+id)}}  docked="bottom"/>  
               </Container>
