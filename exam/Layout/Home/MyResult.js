...
export default class Myresult extends Component {
  ...
  render = () =>{
    if(this.state.exam && this.state.exam && this.state.user_prepay && this.state.exam_user.length>0 && this.state.exam.length>0) {
      for(let iexam in this.state.exam) {
        ...
        if(exam.status<4) delete this.state.exam[iexam] ;
    ...
    var {content} = this.state
      ...
      <Container   style={{margin:"auto"}} maxWidth="800px" maxHeight="100%" minHeight="500px" cls={Ext.platformTags.phone?"rtl":"rtl"}  centered  scrollable height={Ext.platformTags.phone?"100%":""} width="100%">
        ...
        <Button  text="راهنما"  ui="action round" handler={()=>this.setState({helpdialog:true,content:""})}  textAlign="center" width="70px" style={{top:"20px" ,left:"20px", color:"#ffffff"}} />
