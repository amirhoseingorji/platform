  ...
  let gopt=[1,2,3,4,5,6,7,8]
    return <Container padding="10" cls="rtl" layout='fit'cls={Ext.platformTags.phone?"":'examback'}  >
      ...
      <Title text="اطلاعات و راهنمای آزمون " style={{fontSize:"16px"}}  parent={this}/>
        ...
        <div className="rtl" style={{padding:"20px 10px",fontSize:"18px",lineHeight:"38px",textAlign:"justify" , maxWidth:"900px",width:"100%", backgroundColor:"#ffffff" , minHeight:"400px" ,borderRadius:"3px" ,margin:"auto"}} dangerouslySetInnerHTML={{
        ...
        <Container layout={{type:"vbox",align:"center"}}  >
                  ...
                  //   console.log({starter:new Date().getTime()})
                     
                      this.setState({giude:false,questions,duration:(duration-spent)*60,darsha,starter:new Date().getTime()})
                      this.counter = Math.floor((duration-spent)*60)
                      clearInterval(this.int)
                      this.int = setInterval(()=>{
                        let nowchek = new Date().getTime()

                        let remain = Math.floor(this.state.duration -  (nowchek - this.state.starter)/1000)
                        if(this.counter<remain-60) {
                         Ext.toast('تعویق زمان شناسایی شد لطفا مجدد وارد آزمون شوید')
                          this.navigate('/')
                        }
                        if(remain>0 ) {
                          this.counter--
                         // this.state.duration --;
                          this.refs.header.cmp.setHtml('زمان باقیمانده :'+ this.timer(remain))
                          if(remain<600) this.setState({endtime:true})
                        }else {
                          clearInterval(this.int)
...
{/*
1306,693,711,672,130,1109,664,742,1303,720 */}
