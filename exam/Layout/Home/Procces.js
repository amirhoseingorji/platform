...
import { Container, Image, SelectField, Button,CheckBoxField, TextField,PasswordField,SegmentedButton,Dialog,Video,NumberField} from '@sencha/ext-modern';
...
componentWillMount() {
  ...
  phone(){
    ...
    if(this.phoneNumber<9000000000) return Ext.toast('شماره همراه را به صورت صحیح وارد نمایید')
  ...
  passcheck2(){
    ...
    if(pass) window.socket.emit("passnid",this.nid,pass)
  }
  forget(){
  ...
  Login = () =>  {
    return  <Container><Container cls={Ext.platformTags.phone?"backphone":""} padding={Ext.platformTags.phone? "0 25 60":"20"}  height="100%" width="100%" layout="center" >
      ...
      <Button  text="راهنما" docked="left"  handler={()=>this.setState({helpdialog:true})}  textAlign="center" width="70px" ui="action round" />
      </Container>
    ...
    <Container layout="hbox"  shadow maxWidth="800px"  height="100%" width="100%" maxHeight="500px" cls="x-round" style={{backgroundColor:"#ffffff",padding:"0px"}}  >
      ...
      <Button  text="راهنما" ui="action round" hidden={Ext.platformTags.phone} cls="" handler={()=>this.setState({helpdialog:true})} textAlign="center" width="50px" docked="top" style={{color:"#ffffff"}}/>
    ...
    <NumberField labelAlign="top" width="99%"  height="41px"  style={{border:"solid #c4c4c4 1px", borderRadius:"8px",overflow:"visible"}} textAlign="center" label="لطفاشماره&zwnj;ملی&zwnj;خودرا&zwnj;وارد&zwnj;نمایید" id="username"  placeholder="" ref="nid" value={this.phoneNumber} cls="logininput" margin="30 0 15" maxLength={10} />
    <PasswordField labelAlign="top" width="99%" height="41px"  style={{border:"solid #c4c4c4 1px", borderRadius:"8px",overflow:"visible"}} cls="rtl logininput" textAlign="center" margin="10 0 15" label="رمز عبور" ref="PasswordField" onAction={this.passcheck2.bind(this)}   />
    <Container layout="hbox" width="100%"> 

    <Button  text="ورود به سامانه "  ripple ={false} handler={this.passcheck2.bind(this)} flex={1} padding="10 0" margin="5" cls="loginbtn" ripl/>
    <Button  text="ثبت نام"  ripple ={false} handler={this.register.bind(this)} flex={1} padding="10 0" margin="5" cls="loginbtn" />
    </Container>

    {/* <Button {...this.BProps} text="انصراف " iconCls="x-fa fa-ban" handler={()=>this.pather("/")} /> */}
    <Button ref="resentbtn" cls="resentbtn" disabled={this.state.sent} text="رمز عبور خود را فراموش کرده اید؟ اینجا را کلیک کنید" handler={this.forget.bind(this)} textAlign="right"/>
    </Container>
  ...
  Pass = () =>  {
   // if(!this.state.user_info) Storer({ user_info: { 0: { _user:1 } } }, this)
    return <Container><Container cls={Ext.platformTags.phone?"backphone":""} padding={Ext.platformTags.phone? "0 25 60":"20"}  height="100%" width="100%" layout="center" >
            <Container layout="hbox" width="100%" cls="ltr" hidden={!Ext.platformTags.phone}>
      <Button  text="راهنما " docked="left"  ui="action round" handler={()=>this.setState({helpdialog:true})}  textAlign="center" width="70px" />
      </Container>
    <Container layout="hbox"  shadow maxWidth="800px"  height="100%" width="100%" maxHeight="500px" cls="x-round" style={{backgroundColor:"#ffffff",padding:"0px"}}>
    <Container flex={2} layout={{type:"vbox",pack:"start",align:"center"}} style={{margin: Ext.platformTags.phone? "10px 15px 40px":"30px 50px 100px"}}  >
      <Container layout="hbox" width="100%">
      <Button  text="راهنما" ui="action round" hidden={Ext.platformTags.phone} cls="" handler={()=>this.setState({helpdialog:true})} textAlign="center" width="50px" docked="left" style={{color:"#ffffff"}}/>
    ...
    <NumberField labelAlign="top" width="99%" height="41px"   style={{border:"solid #c4c4c4 1px", borderRadius:"8px",overflow:"visible"}} textAlign="center" label="لطفاشماره&zwnj;همراه&zwnj;خودرا&zwnj;وارد&zwnj;نمایید"  onAction={this.phone.bind(this)} placeholder="09000000000" ref="phone" value={this.phoneNumber} cls="logininput" margin="30 0 15" maxLength={12} />
 <NumberField labelAlign="top" width="99%" height="41px"  style={{border:"solid #c4c4c4 1px", borderRadius:"8px",overflow:"visible"}} textAlign="center" label="لطفاشماره&zwnj;ملی&zwnj;خودرا&zwnj;وارد&zwnj;نمایید"   placeholder="" ref="nid" value={this.nidNumber} cls="logininput" margin="5 0 15" maxLength={10} />
     
    <Button  text="دریافت رمز عبور" ripple ={false} handler={this.phone.bind(this)} width="99%" padding="10 0" margin="auto" cls="loginbtn" />
    </Container>
  ...
  saveinfo() {
     var data = {}
     var newd= {}
     newd = this.getvals({ paye:"",first_name:"",last_name:"",_province:"", _province_city:"", school_name:"", moaref_name:"", class_name:""})
  ...
  Info = () =>  {
    ...
    if(!this.state.user_info){ 
      ...
      province_city:{},
      user_info:{join:"user",cols:"user.paye,user.nationalid,user.user,sex,first_name,last_name,user.name,_province,_province_city,school_name,class_name,moaref_name"}
...
<Container layout={Ext.platformTags.phone?"vbox":"hbox"} >
  ...
  <TextField ref="nationalid"  {...this.TPropsn} label="کد ملی" value={user_info.nationalid}  disabled />
  <TextField ref="phonenumber"  {...this.TPropsn} label="شماره همراه" value={user_info.user}  disabled />
