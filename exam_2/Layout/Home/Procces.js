...
componentWillMount() {
      ...
      if(sent) {
        window.int = setInterval(()=>{
          if(this.refs.resentbtn) 
               this.refs.resentbtn.cmp.setText("دریافت مجدد رمز" +  "   (" + (this.counter--)+")");
               else clearInterval(this.window)
        },1000)
        setTimeout(()=>{
          clearInterval(window.int)
        this.setState({sent:false})
        this.refs.resentbtn.cmp.setText("دریافت مجدد رمز");
  ...
  Login = () =>  {
    return  <Container padding="7"><br/><Image src='https://andishmand.ir/wp-content/uploads/2019/07/logo.png' mode='img' width="180" style={{margin:"auto"}} />
    <center > سامانه آزمون آنلاین اندیشمند  </center><br/><Container {...this.MainProps}  >
    {/* <Title icon='sign-in' text="ورود / ثبت نام" home={false}/> */}
    ...
    <NumberField {...this.TProps} label="شماره همراه" placeholder="90123456789" ref="phone" onAction={this.phone.bind(this)} value={this.phoneNumber||""}/>
    
    <Button {...this.BProps} text="ادامه" iconCls="x-fa fa-mobile-android-alt" handler={this.phone.bind(this)} width="99%" margin="auto" />
  ...
  Pass = () =>  {
   // if(!this.state.user_info) Storer({ user_info: { 0: { _user:1 } } }, this)
    ...
    <Button {...this.BProps} text="تایید " iconCls="x-fa fa-key" handler={this.passcheck.bind(this)} width="95%"/>
    <Container layout='hbox' >
    <Button ref="resentbtn"  disabled={this.state.sent} text="دریافت مجدد رمز" iconCls="x-fa fa-refresh" handler={this.phone.bind(this)} />
    <Button  text="تغییر شماره"  handler={()=>this.router('Login')}/>
  ...
  saveinfo() {
     var data = {}
     var newd= {}
     newd = this.getvals({ paye:"",sex:"",first_name:"",last_name:"",	nationalid:"",_province:"", _province_city:"", school_name:"", class_name:""})
    ...
    Storer("user_", this,{from:"user",paye:newd.paye,name : newd.first_name+" "+newd.last_name} , "update");
     data = {...data,...newd,name:newd.first_name+" "+newd.last_name}
     data.from = "user_info"
  ...
  Info = () =>  {
    ...
    if(!this.state.user_info){ Storer({ 
    ...
    let user_info =  this.state.user_info.length ?  this.state.user_info[0] :
     { paye:1,sex:1,
       ...
       _province:"", _province_city:"", school_name:"", class_name:""
    }
    ...
    this.maxstep = 1
    return <Container {...this.MainProps} height="calc(100%)" layout="vbox">
    <Panel  activeItem={step} layout="card" flex={1} >

        <Container scrollable width="95%">
          <Container layout={{ type: 'hbox', pack: 'space-around'  }} width="100%" style={this.headstyle}><Container margin="10 90 0 0">{" تکمیل مشخصات فردی "}</Container><Button
        margin="3"
        docked="right"
           text="تراکنش ها"
           style={{color:"#ffffff" , fontSize:"10px"}}
            handler={()=>this.pather("/Transactions")}
            iconAlign="right" />
          </Container>
          ...
          <SelectField ref="sex" label="جنسیت" {...this.TProps} value={user_info.sex||1} picker={"floated"}  required options={[
            { text: 'دختر', value: 0 },
            { text: 'پسر', value: 1 },
          ]} onChange={(a, b, c) => { 
            this.setState({ sex:b}) }} />
          ...
          <TextField ref="class_name" validateOnInit="all" {...this.TProps} label="نام کلاس"  value={user_info.class_name}   inputMask= 'اااااااااااااااااا'/>
            ...
            <Container docked="bottom" layout="hbox" cls="ltr">
...
onIndicatorTap = (indicator, dot) => {
  ...
  Transactions = () => {
    if(!this.state.user_pay) {Storer({ user_pay:{store:true,refresh:true,cols:"`id`, `_user`, amount, `cardNumber`, `transactionId`, `date`"}}, this); return null} else
    return <Container {...this.MainProps} height="98%"  >
    <Grid layout="fit" height="99%" rowNumbers store={this.state.user_payStore}    plugins={{ gridpagingtoolbar: false, gridsummaryrow: true          }} >
        <TitleBar title="تراکنش ها" docked="top" ui="titlebar-search" cls="x-round">
          <Icon name="exchange" align="left" />
           <Button {...this.BProps} iconCls="x-fa fa-arrow-left" ui="alt round" shadow margin="0" align="right" handler={()=>this.pather("/")}/> 
        </TitleBar>
      <Column text=" شماره کارت" flex="2" dataIndex="cardNumber" align="center" summaryRenderer={()=>"جمع"}/>
      <Column text="پیگیری" flex="1" dataIndex="transactionId" align="center"/>
      <Column text="تاریخ" flex="1" dataIndex="date"   align="center" renderer={v=>moment(v).format("jM/jD HH:mm")}/>
      {/* <Column text="فاکتور مرجع" dataIndex="_user_factor" renderer={this.transkey.bind(this)} summaryRenderer={()=>""}/> */}
      <Column text="مبلغ" flex="1" dataIndex="amount"  summary="sum" align="center"/>
    </Grid>
  </Container>
  }
  printTransactions = () => {
  ...
  MainProps = {
  //  cls: 'rtl',
    shadow: true,//!Ext.platformTags.phone ,
    cls: 'rtl x-round',//Ext.platformTags.phone ? "rtl":'rtl x-round',
