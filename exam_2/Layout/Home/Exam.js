  ...
  let gopt=[1,2,3,4,5,6,7,8]
    return <Container padding="10" cls="rtl" layout='fit' >
      ...
      <Title text="اطلاعات و راهنمای آزمون " style={{fontSize:"16px"}}  parent={this}/>
        ...
        <div  style={{padding:"20px 10px",fontSize:"18px",lineHeight:"38px",textAlign:"justify" ,backgroundColor:"#ffffff" , borderRadius:"3px" ,margin:"15px 0"}} dangerouslySetInnerHTML={{
...
{this.state.questions.map((e,i)=><Container  maxWidth="450" cls='rtl' key={i} ><div></div><div id={"qu"+e.number}>سوال شماره {e.number}:</div>
<div style={{backgroundColor:"#ffffff",width:"96%", margin:"0 0 5px",minHeight:"500px"}} >
...
{gopt.slice(0,e.options).map((c,ii)=><RadioField cls="x-center"  flex={1} name= {'radio'+e} key={i*1+ii} boxLabel={c} checked={c==e.user_answer?true:false}  Width="100%" ripple={false} margin="0 5px" style={{fontSize:"18px" ,backgroundColor:"#ffffff" ,borderRadius:"5px"}} onFocus={(th)=>{
...
{setTimeout(()=>{
  ...
  setTimeout(()=>{th.setChecked(false)},10)
}
}}      onCheck={(th)=>{
  window.socket.emit("change",{_exam_user: this._exam_user ,number:e.number,user_answer:c})
  window.socket.on("change",(p)=>{
    th.setChecked(true)
    window.socket.off("change")
    //Ext.toast(p)
  })
