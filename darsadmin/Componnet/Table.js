...
import { Container, Grid, TitleBar, Column, NumberField,Button,SliderField,Image, FileField,Dialog,TextAreaField,TimeField,DatePickerField,SelectField ,Video, SearchField,ActionSheet,Progress ,Rating } from '@sencha/ext-modern';
                ...
                <Container  {...this.MainProps} ref="dragContainer" margin={0}   >
                    ...
                    ref={grid => this.grid = grid}>
                        <TitleBar title={title} docked="bottom"    > 
                            ...
                            <SelectField hidden={true} value={filteropt[0].value} ref="_filterKey"  options={filteropt} {...this.Iprops} onChange={(a,b)=>{this.setState({selectedFilter:b})}} ripple={false}/>
                            <Button {...this.BProps2} hidden={true}  text="بگرد" handler={this.filterer.bind(this)} ripple={false} margin="0 5 0 100" width="80"/>
                            </Container>
                            ...
                            if(e[1].btn) {
                                ...
                                e[1].renderer = (value,record,column)=>this[e[1].btn](value,record,column,e[1].options,e[1].event);
                                    ...
                                    return   <span column={e[1].dataIndex||e[0]} className={icon}  style={{cursor:"pointer",width:e[1].width+"px",color:"#555555"}} onClick={(a)=>{
                                        ...
                                        this.navigate(path+(path.indexOf("?")>0?"&":"?")+col+"="+id+(group?"&group="+group:""))
                                    ...
                                    return <div >
                                    <DatePicker   isGregorian={false}  value={moment(a)} onChange={(a,b)=>{
                                        if(b && b.input) {
                                            console.log(b.input.closest(".x-gridrow"))
