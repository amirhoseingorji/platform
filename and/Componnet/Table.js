...
import { Tree, TreeColumn ,CheckColumn} from '@sencha/ext-modern-treegrid';
...
export default class Table extends Component {
    ...
    componentWillMount() {
      ...
      //  let where = 
        var data= {cols, from: this.props.from, store: this.props.tree?"tree":true, refresh: true, num:this.num,page:this.page,total:true,order:this.order,asc:this.asc  }
   
        
        if(_where) data.where = _where;
        else  {

            if(this.props[0]) data[0] = {...query,...this.props[0]}; else if(Object.keys(query).length>0) data[0] = query;
        ...
        if(this.props.refresh && typeof this.props.refresh == "function") this.props.refresh()
         if(this.props.idProperty) data.idProperty = this.props.idProperty;
      ...
      // if(!this.state) for(let col in _cols) if(_cols[col].options && _cols[col].options.from && !this.state[tables[col]]) tables[col] = _cols[col].options;
        Storer({data}, this);
        // setTimeout(()=>{
        //     if(this.props.tree) this.grid.cmp.expandAll();
        // },1000)
    ...
    delrec(store,record){
        if(this.props.tree && record.childNodes && record.childNodes.length>0) for(let rec of record.childNodes) this.delrec(store,rec)
        store.remove(record);
    }
    delete = (record,confirm=false,event)=>{
        ...
        if(confirm==false) return Ext.Msg.confirm("اخطار!","آیا از حذف این مورد اطمینان دارید؟",(st)=>{if(st=="yes") this.delete(record,true,event); });
            ...
            console.log(_me,record)
            this.delrec(_me,record)
           // _me.remove(record);
            this.updateOrd()
    ...
    new =()=>{
        ...
        if(this.props.tree && this.grid.cmp._selectable._selected.items.length) data.pid = this.grid.cmp._selectable._selected.items[0].id
   ...
   whandler=(button)=>{
    ...
    // htmlbtn=(Options,event)=>{
    //     return {
    //         ...this.BProps,
    //         ui:"confirm",
    //     shadow:false,
    //     cls:"rtl",
    //     iconCls:"x-fas fa-file-edit" ,
    //     //scale:"180%",

    //     margin:0,
    //     //handler : (b)=>{let {value,record,column} = this.whandler(b);this.gethtm(value,record,column,Options)}
    // }}
...
// }
    ...
    updateitem(item,i){
        if(this.props.cols.ord) item.set("ord",i); 
        if(this.props.tree) {
            item.set("pid",item.parentNode.id=="root"?"{null}":item.parentNode.id); 
            for(let j in item.childNodes) this.updateitem(item.childNodes[j],j)
        }
    }
    updateOrd(node, data) {
        
        for (let i in this.grid.cmp.store.data.items) {
            this.updateitem(this.grid.cmp.store.data.items[i], i)
        }
...
//gridexporter: true,
        ...
        if(this.props.tree) {
            plugins.treedragdrop = {dragText:"تغییر ترتیب"},
        ...
        return <Container layout="fit" hidden={this.props.hidden} width={this.props.width||"100%"} padding={0}>
            ...
            <Container cls="rtl" layout={{ type: "vbox", align: Ext.platformTags.phone ? "strech" : "center", pack: "start" }} layout="fit" margin={this.props.noshowddow?0:5} scrollable ref="mainPage" padding="0" >
                ...
                <Container  {...this.MainProps} ref="dragContainer" margin={0} cls={this.props.noshowddow?"":"x-round"}  >
                    ...
                    <MGrid  selectable={{mode:"single"}} store={this.state.dataStore} shadow={this.props.noshowddow?false:true} cls={this.props.noshowddow?"":"x-round"} plugins={plugins} sortable={!this.props.tree && !this.props.unsortable}  onColumnSort={this.onsort.bind(this)} 
                    columnMenu ={null} 
                    ...
                    ref={grid => this.grid = grid}>
                        <TitleBar title={title} docked="top" ui="titlebar-search" cls={this.props.noshowddow?"":"x-round"}  > 
                            ...
                            <Button {...this.BProps2} hidden={!this.props.new}   iconCls="x-fa fa-plus-square" handler={this.new}  text={this.props.newtext||""} ripple={false} width={this.props.newtext?"":35}/>
                        ...
                        {Object.entries(cols).map((e, i) => {
                            if(e[1].btn) {
                                ...
                                e[1].renderer = (value,record,column)=>this[e[1].btn](value,record,column,e[1].options,e[1].event);
                                    ...
                                    e[1].renderer=(a,r)=>{
                                    let o =e[1].options
                                    let icon = o&&o.icon?o.icon:"x-fas fa-link"
                                    
                                    return<span column={e[1].dataIndex||e[0]} className={icon+(a?" blue":" deactive")}  style={{cursor:"pointer",width:e[1].width+"px",color:"#555555"}} onClick={(a)=>{
                                    ...
                                    e[1].renderer=(a)=>{
                                        if(a) return <img column={e[1].dataIndex||e[0]} src={a}  height="50" margin="" style={{objectFit: "contain",cursor:"pointer"}} onClick={(a)=>{
                                        let record = this.state.dataStore.data.items[a.target.closest(".x-gridrow").getAttribute("data-recordindex")];
                                        let column = a.target.getAttribute("column");
                                        let value = record.data[column];
                                    ...
                                    else return <div column={e[1].dataIndex||e[0]}    className={"x-fas fa-camera-retro"+(a?" blue":" deactive")}  style={{cursor:"pointer",height:"35px",width:e[1].width+"px",color:"#555555", marginTop:"15px"}} onClick={(a)=>{
                                        let record = this.state.dataStore.data.items[a.target.closest(".x-gridrow").getAttribute("data-recordindex")];
                                        let column = a.target.getAttribute("column");
                                        let value = record.data[column];
                                        console.log(value,record,column)
                                    ...
                                    e[1].renderer=(a)=>{
                                    let icon = a.length<10?"file-edit deactive":"file-edit blue"
                                    return <span column={e[1].dataIndex||e[0]} className={"x-fa fa-"+icon}  style={{cursor:"pointer",width:e[1].width+"px",color:"#555555"}} onClick={(a)=>{
                                        let record = this.state.dataStore.data.items[a.target.closest(".x-gridrow").getAttribute("data-recordindex")];
                                        let column = a.target.getAttribute("column");
                                        let value = record.data[column];
                            ...
                            let MColumn =e[0]==this.props.tree? TreeColumn : (e[1].type=="check" ? CheckColumn : Column)
                            return e[1].text  ? <MColumn key={i} dataIndex={e[0]} {...e[1]} align={e[0]==this.props.tree?"right":"center"}   >
                                ...
                                {e[1].type == "date" ? <DatePickerField cls='ltr'  /> : null}
            ...
            >
                ...
                <Container cls="printdialog rtl" style={{color:"#000000"}} layout="hbox" >
                   ...
                   {imgpath && imgpath.length>1 && <Image src={imgpath} mode="img" height="400"  style={{objectFit: "contain"}} />}
                </Container>
                    <Video ref="_Video" height="400" maxWidth="400" width="100%"  url={[mp4path]} hidden={!mp4path || mp4path.length<2} />
    ...
    uploader(){
        ...
        var reader  = new FileReader();
         reader.addEventListener("load",  ()=>{
          ...
          socket.emit('uploadchuk', { 
              ...
              part,
              percent,
              data: reader.result.slice(part*chunkSize,(part+1)*chunkSize)
          });
          window.socket.on("uploadchuk",(part)=>{
              percent = Math.floor(100*part*chunkSize/reader.result.length)
             console.log({percent})
            socket.emit('uploadchuk', { 
                name: (this.state.Options.path||"") + file.name.toLowerCase(), 
                type: file.type, 
                size: file.size,
                part,
                percent,
        ...
        if (file)  if(file.name.toLowerCase().indexOf(".fbx")>-1) reader.readAsBinaryString(file);else reader.readAsDataURL(file);
    }
