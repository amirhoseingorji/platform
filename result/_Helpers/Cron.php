...
//echo "cron class loaded";
    class Cron{
		private $folder;
		private $master;
		function __construct($master=__FILE__){//ok
			$this->master = $master;
			$this->folder = __DIR__."/".str_replace(".php","",basename($this->master))."_Cron";
		...
		private function setCron($file,$time){
			...
			$file = "http://".$_SERVER["SERVER_NAME"]."/_Controllers".substr($_SERVER['REQUEST_URI'],0,-4)."_Cron/".basename($file);
