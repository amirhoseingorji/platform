...
class home{
    ...
    public function model_2($fname , $lname , $telegramId , $chatId , $phone){
		if($this->model_1($chatId)>0) return false;
        $user = $this->db->user->insert([
            'id'   => $chatId ,
            'name' => ' '.$fname.' '.$lname
        ]);
		if(is_numeric($user)==false) return false;
    ...
    public function model_6($_exam_user , $question , $msgId=0){
        $par =[];
        $par['_exam_user'] = $_exam_user;
        $par[0]['number'] = $question;
        $this->db->exam->user->answer->update(
            ['add_time' => '`id`-`id`+now()' , 'msgId' => $msgId] , $par
        );
        $this->db->exam->user->update(
            ['showcount' => $question],
            ['id' => $_exam_user]
    ...
    public function model_7($_exam_user , $question){
        ...
        if($listExam[0]['add_time'] == NULL and $status<2){
			$this->model_6($_exam_user , $question);
...
AS sorted USING(id) SET exam_user.rank = sorted.rank" , "");
			$this->db->query("update `exam_user` INNER JOIN (SELECT balance,MIN(rank) as irank FROM `exam_user` GROUP by `balance`) as ins ON `exam_user`.balance = ins.balance set `exam_user`.`rank` = ins.irank WHERE _exam_item =".$examItemes['id'], "");
        }
...
//calc total balance for each user and rankes
		...
		$this->db->query("update `exam_user` INNER JOIN (SELECT balance,MIN(rank) as irank FROM `exam_user` GROUP by `balance`) as ins ON `exam_user`.balance = ins.balance set `exam_user`.`rank` = ins.irank WHERE _exam_item =".$masterExamItem, "");
...
//calc avg for each question
    ...
    public function model_17_1($_exam,$status){
			...
			$par[0][0]['pid'] = 0;
			$par[1]['status'] = $status;
			$par['cols'] = 'exam_user.id exam,_user,name,start_time,workbook,if(begin_time >0,1,0) as realst';
    ...
    public function model_17_2($_exam,$user,$file){
		...
		$par['workbook'] = $file;
		$wer = [];
		$wer['_exam_item'] = $exam_item;
		$wer[0]['_user'] = $user;
        $this->db->exam->user->update($par,$wer);
    }
