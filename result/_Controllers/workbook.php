...
class workbook 
{
	public function __construct($model){
        ...
        //print_r($this->jdate->date("y/m/d",strtotime($getExam['start_time'] - 24*60*60)));
        //print $getExam['name'].' : '.$this->jdate->date("y/m/d",strtotime($getExam['start_time'])- 24*60*60);
		...
		$this->point 		= $this->model->point($this->userId, $this->azmonId);//[410,0,0,0];
		$list = [];
		$chartDate = $this->model->chartdate($this->userId, $this->azmonId);
		foreach ($chartDate as $chartDater) {
			array_push($list , $chartDater['name']);//.' : '.$this->jdate->date("m/d",strtotime($chartDater['start_time'])));
		}
		$this->date 		= $list;
