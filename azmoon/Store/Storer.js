...
const Storer = (Table, _this, config = {}, mode = "select") => {
    if (typeof Table == "object") {
        ...
        if (config.store) {
            ...
            let idProperty = config.idProperty || "id"
        ...
        if(_this) _this.setState({..._thisstate})
        if(_this) if(_this.refs[Table]){
            _this.refs[Table].setState({loading:true})
        }
        if(_this) then.push((data,count) => {
            if(count &&  data) data.count = count;
            state[Table] = data
